...
#define HUD_MAG_STORE_BYTES 8

/** And the mounting calibration, straight after the compass. */
#define HUD_MOUNT_STORE_OFF   (HUD_MAG_STORE_OFF + HUD_MAG_STORE_BYTES)
#define HUD_MOUNT_STORE_BYTES 16
// hud_mount.h has its own HUD_MOUNT_BYTES for the same buffer, because it must
// compile standalone for the host tests. Two independent numbers for one array
// is a stack smash waiting for whoever changes one of them, so it is a build
// error instead. (Checked there rather than here: this header is included
// first and does not know about the other one.)
