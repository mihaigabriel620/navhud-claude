...
//  data types it instantiates (HudCanvas, CarState, HudImu) and before every
//  module that reads them -- the themes read `car` and `tft` directly.
// ---------------------------------------------------------------------------
#ifndef HUD_STATE_H
#define HUD_STATE_H
...
#ifdef HUD_IMU
extern HudImu    imu;
extern uint32_t  lastImuSendMs;
#endif
#ifdef HUD_MAG
/** The compass. Its own object now: it does not live behind the IMU. */
extern HudMag    mag;
extern float     magHeadingDeg;
extern uint32_t  lastMagSendMs;
/** Which way the sensor is bolted in. See hud_mount.h. */
extern HudMount  mount;
/**
 * Calibrations waiting to be written: bit 0 the compass, bit 1 the mounting.
 *
 * Deferred rather than written where they are asked for. The typed commands
 * are handled from inside the serial read loop, and EEPROM.commit() on an
 * ESP8266 erases and rewrites a whole 4 KB flash sector with interrupts off --
 * 30 to 45 ms typically and up to 400 ms by the datasheet. Nothing fills the
 * UART receive buffer in that window, so everything the phone sent during it
 * is gone, and a $CAM or $LANE clearing frame lost there is lost for good
 * because both are edge-triggered. So the flag is set there and the write
 * happens from loop(), at a standstill.
 */
extern uint8_t   pendingSave;
#endif
